...
public class DatabaseAdapter {
	...
	public List<SearchedItem> getAllHotels() {
		...
		c = databaseHolder.query(true, DB_TABLE_ITEM, new String[] {
				KEY_ITEM_NAME, KEY_ITEM_IMAGE, KEY_ITEM_PRICE_FROM,
				KEY_ITEM_PRICE_TO, KEY_ITEM_DESCRIPTION, KEY_ITEM_RATING,
				KEY_ITEM_LAT, KEY_ITEM_LONG, KEY_ITEM_LOCATION }, "WHERE "
				+ KEY_ITEM_TYPE + " = 'hotel'", null, null, null, null, null);
	...
	public Cursor getAllTouristDestination() {
		return databaseHolder.query(true, DB_TABLE_ITEM, new String[] {
				...
				KEY_ITEM_PRICE_TO, KEY_ITEM_DESCRIPTION, KEY_ITEM_RATING,
				KEY_ITEM_LAT, KEY_ITEM_LONG, KEY_ITEM_LOCATION }, "WHERE "
				+ KEY_ITEM_TYPE + " = 'destination'", null, null, null, null,
				null);
	}
	...
	public Cursor getHotelAmneties(String hotel_id) {
		...
		c = databaseHolder.query(true, DB_TABLE_ITEM_AMNETIES,
				new String[] { KEY_ITEM_AMNETIES_AMNETY_ID }, "WHERE "
						+ KEY_ITEM_AMNETIES_ITEM_ID + " = " + hotel_id, null,
				null, null, null, null);

		amnety_id = c.getString(0);

		return databaseHolder.query(true, DB_TABLE_AMNETY,
				new String[] { KEY_AMNETY_NAME }, "WHERE " + KEY_ID + " = "
						+ amnety_id, null, null, null, null, null);
	}
	...
	private static class DBHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			...
			db.execSQL("CREATE TABLE " + DB_TABLE_ITEM + " (" + KEY_ID
					...
					+ " text not null, "
					+ KEY_ITEM_IMAGE
					+ " text not null, " // drawable
					+ KEY_ITEM_PRICE_FROM + " int not null, "
					+ KEY_ITEM_PRICE_TO + "int, " + KEY_ITEM_LOCATION
					+ " text not null, " + KEY_ITEM_DESCRIPTION
					+ " text not null, " + KEY_ITEM_RATING + " int null, "
					+ KEY_ITEM_LAT + " number, " + KEY_ITEM_LONG + " number");
